...
const GanttTimeline = ({ tasks, startDate }: Props) => {
  ...
  const { bars, totalDays, milestones } = useMemo(() => {
    ...
    const bars: GanttBar[] = tasks.map((t) => {
      ...
      const bar: GanttBar = { id: t.id, title: t.title, days, startDay: offset, completed: t.completed };
      offset += days;
      return bar;
    });
    const totalDays = offset;
    const milestones: number[] = [];
    for (let d = 5; d < totalDays; d += 5) milestones.push(d);
    ...
    <div className="premium-card p-6 space-y-5">
      ...
      <div className="flex gap-4 text-xs text-muted-foreground">
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-2.5 w-5 rounded" style={{ background: COLORS.completed }} />
          Done
        </span>
        <span className="flex items-center gap-1.5">
          <span className="inline-block h-2.5 w-5 rounded" style={{ background: COLORS.active }} />
          Remaining
        </span>
      </div>
      ...
      <div className="flex overflow-hidden rounded-xl border bg-background">
        <div className="shrink-0 border-r bg-secondary/30" style={{ width: LABEL_WIDTH }}>
          ...
          {bars.map((bar) => (
            ...
            >
              {bar.completed && <CheckCircle2 className="h-3 w-3 text-success shrink-0" />}
              <span className={`text-xs truncate ${bar.completed ? "text-muted-foreground line-through" : "text-foreground"}`}>
                {bar.title}
              </span>
            </div>
          ))}
              ...
              {bars.map((bar, i) => {
                ...
                return (
                  <g key={bar.id} onMouseEnter={() => setHoveredId(bar.id)} onMouseLeave={() => setHoveredId(null)}>
                    <rect x={x} y={y} width={w} height={BAR_HEIGHT} rx={8} fill={fill}
                      opacity={isHovered ? 1 : 0.85} className="transition-opacity duration-150" />
                    {w > 30 && (
                      <text x={x + w / 2} y={y + BAR_HEIGHT / 2} textAnchor="middle" dominantBaseline="central"
                    ...
                    {isHovered && (
                      <g>
                        <rect x={x + w / 2 - 60} y={y - 28} width={120} height={22} rx={6}
